...
class AriesProcessor(ProcessorMixin):
    r"""
    Constructs a ARIES processor which wraps a LLama tokenizer and ARIES image processor into a single processor.

    [`IdeficsProcessor`] offers all the functionalities of [`AriesImageProcessor`] and [`LlamaTokenizerFast`]. See
    the docstring of [`~IdeficsProcessor.__call__`] and [`~IdeficsProcessor.decode`] for more information.

    Args:
        image_processor (`AriesImageProcessor`):
            An instance of [`AriesImageProcessor`]. The image processor is a required input.
    ...
    image_processor_class = "AriesImageProcessor"
    ...
    def __call__(
        ...
        ```python
        >>> import requests
        >>> from transformers import AriesProcessor
        >>> from transformers.image_utils import load_image

        >>> processor = AriesProcessor.from_pretrained("HuggingFaceM4/aries-8b", image_seq_len=2)
    ...
    def apply_chat_template(
        ...
        **kwargs,
        ...
        """
        Overrides the tokenizer's `apply_chat_template` method to apply the ARIES chat template by default
